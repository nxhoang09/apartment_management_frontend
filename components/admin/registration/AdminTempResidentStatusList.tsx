...
export function AdminTempResidentStatusList({ status, title, showActions = false, keyword }: Props) {
  const { token } = useAuth()
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setError(null)
      try {
        const res = await adminGetPendingTempResidents(token ?? undefined, { status, page, limit: pageSize, sortBy, order, keyword })
  ...
  return (
    <Card>
      ...
      {showActions && (
        <ReviewTempResidentDialog open={dialogOpen} registrationId={selectedId} onClose={() => setDialogOpen(false)} onUpdated={onUpdated} readOnly={true} />
      )}
    </Card>
  )
}
