...
export function AdminTempResidentStatusList({ status, title, showActions = false }: Props) {
  const { token } = useAuth()
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setError(null)
      try {
        const res = await adminGetPendingTempResidents(token ?? undefined, { status, page, limit: pageSize, sortBy, order })
