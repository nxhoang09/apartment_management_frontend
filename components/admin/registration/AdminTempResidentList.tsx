...
import { adminGetPendingTempResidents } from "@/lib/api/registration/registrationApi"
import { useAuth } from "@/lib/context/auth-context"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table"
import { Button } from "@/components/ui/button"
import { format } from "date-fns"
import { ReviewTempResidentDialog } from "./ReviewTempResidentDialog"
...
export function AdminTempResidentList({ keyword }: Props) {
  ...
  // Pagination / page size (declare before effects that use them)
  ...
  const handleReject = (id: number) => {
    // open dialog to allow entering reject reason
    setSelectedId(id)
    setDialogOpen(true)
  }
  ...
  // estimate row height (px) to reserve space for up to `pageSize` rows to avoid layout jump
  const rowHeight = 56
  const tableMinHeight = pageSize * rowHeight
  ...
  return (
    <Card>
            ...
            <div className="overflow-x-auto">
              ...
              <TableHeader>
                <TableRow>
                  ...
                  >
                    Mã hộ {sortBy === "householdId" ? (order === "asc" ? "▲" : "▼") : ""}
                  </TableHead>
              ...
              <TableBody>
                {displayed.map((it) => (
                  <TableRow key={it.id}>
                    ...
                    <TableCell className="whitespace-nowrap text-center">{it.householdId ?? "-"}</TableCell>
      ...
      <ReviewTempResidentDialog open={dialogOpen} registrationId={selectedId} onClose={() => setDialogOpen(false)} onUpdated={onUpdated} />
    </Card>
  )
}
