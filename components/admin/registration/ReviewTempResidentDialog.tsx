...
import { adminGetTempResidentDetail, adminApproveTempResident, adminRejectTempResident } from "@/lib/api/registration/registrationApi"
...
export function ReviewTempResidentDialog({ open, registrationId, onClose, onUpdated, readOnly }: Props) {
  const { token } = useAuth()
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null)
      try {
        const res = await adminGetTempResidentDetail(registrationId!, token ?? undefined)
  ...
  const handleApprove = async () => {
    ...
    setError(null)
    try {
      await adminApproveTempResident(registrationId, token ?? undefined)
      onUpdated?.()
      onClose()
    } catch (err: any) {
  ...
  const handleReject = async () => {
    ...
    setError(null)
    try {
      await adminRejectTempResident(registrationId, rejectReason, token ?? undefined)
      onUpdated?.()
      onClose()
    } catch (err: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-[700px]">
        <DialogHeader>
          <DialogTitle>Chi tiết & Duyệt đơn</DialogTitle>
          ...
          <div className="text-sm">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <div className="font-medium">{detail?.householdId ?? "-"}</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-2">
              ...
              <div>
                ...
                <div className="font-medium">{detail?.resident?.dateOfBirth ? format(new Date(detail.resident.dateOfBirth), "dd/MM/yyyy") : "-"}</div>
              </div>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-2">
              <div>
                <div className="text-xs text-muted-foreground">Quan hệ</div>
                <div className="font-medium">{detail?.resident?.relationshipToHead ?? "-"}</div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Nơi sinh</div>
                <div className="font-medium">{detail?.resident?.placeOfOrigin ?? "-"}</div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Nghề nghiệp</div>
                <div className="font-medium">{detail?.resident?.occupation ?? "-"}</div>
              </div>
            </div>
